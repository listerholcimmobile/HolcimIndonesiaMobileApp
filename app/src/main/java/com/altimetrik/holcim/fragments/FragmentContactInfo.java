    ...
    public void refreshImage() {
        try {
            if(saleExecution.getContactSFId()!=null) {
                ...
                if(contact!=null) {
                    if (contact.isPictureTempFileExist(getActivity())) {
                        camera.setImageBitmap(HolcimUtility.decodeSampledBitmapFromFile(contact.getTempPictureFilePath(getActivity()), 200, 200, 0));
                    } else if (contact.getId() != null && contact.isPictureFileExist(getActivity())) {
                        camera.setImageBitmap(HolcimUtility.decodeSampledBitmapFromFile(contact.getPicturePath(getActivity()), 200, 200, 0));
